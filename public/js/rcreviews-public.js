jQuery( document ).ready(function($) {
	...
	$('.rcreviews--read-more').on('click', function() {
		$(this).parent().prev().removeClass('rcreviews--truncate');
		$(this).remove();
	});

	$('.rcreviews--btn').on('click', function() {
		$('.rcreviews--hidden-review').toggleClass('d-none');
		$('.rcreviews--label').toggleClass('active');
		$('.rcreviews--count').toggleClass('d-none');

		if ( $('.rcreviews--label').hasClass('active') ){
			$('.rcreviews--label').text('Show less');
		} else{
			$('.rcreviews--label').text('Show');
		}
	});
